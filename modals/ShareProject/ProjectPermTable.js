...
const RowGrid = styled.div`
  align-items: end;
  display: grid;
  grid-template-columns: minmax(190px, 1fr) repeat(3, 50px);
  grid-gap: 2px;
  padding-right: 8px;
...
export default class ProjectPermTable extends PureComponent {
  static propTypes = {
  ...
  render() {
    const {
      columns,
      effectivePermissions: { contentCreator },
      items,
      onInvite,
      onTogglePerm,
    } = this.props;
    return (
      <ListGrid>
        ...
        {items.map(i => {
          ...
          // can grant/revoke contentCreator if user has that perm
          // can only revoke read if that doesn't mean revoking a perm the user dosn't have
          const canChangeRead = !i.read || (contentCreator || !i.content);
