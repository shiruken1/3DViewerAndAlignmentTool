// https://raw.githubusercontent.com/sloisel/numeric/master/src/svd.js

function rep(s, v, k) {
  const n = s[typeof k === 'undefined' ? 0 : k];
  const ret = Array(n);
  let i;
...
export default function svd(A) {
  ...
  function pythag(oldA, oldB) {
    const a = Math.abs(oldA);
    const b = Math.abs(oldB);
  ...
  for (k = n - 1; k !== -1; k += -1) {
    for (let iteration = 0; iteration < itmax; iteration++) {
      ...
      if (iteration >= itmax - 1) throw new Error('Error: no convergence.');
